...
/**
 * Cache responses in a directory on the file system. Most clients should use {@code android.net.HttpResponseCache}, the stable, documented front end
 * for this.
 */
public final class HttpResponseCache extends ResponseCache implements OkResponseCache {
	...
	/**
	 * Handles a conditional request hit by updating the stored cache response with the headers from {@code httpConnection}. The cached response body
	 * is not updated. If the stored response has changed since {@code conditionalCacheHit} was returned, this does nothing.
	 */
	...
	public void update(CacheResponse conditionalCacheHit, HttpURLConnection httpConnection) throws IOException {
		...
		RawHeaders varyHeaders = httpEngine.getRequestHeaders().getHeaders().getAll(response.getVaryFields());
		Entry entry = new Entry(uri, varyHeaders, httpConnection);
		DiskLruCache.Snapshot snapshot = (conditionalCacheHit instanceof EntryCacheResponse) ? ((EntryCacheResponse) conditionalCacheHit).snapshot
	...
	private static final class Entry {
		 ...
		 * </pre>
		 * 
		 * The file is newline separated. The first two lines are the URL and the request method. Next is the number of HTTP Vary request header
		 ...
		 * 
		 * <p>
		 * HTTPS responses also contain SSL session information. This begins with a blank line, and then a line containing the cipher suite. Next is
		 * the length of the peer certificate chain. These certificates are base64-encoded and appear each on their own line. The next line contains
		 * the length of the local certificate chain. These certificates are also base64-encoded and appear each on their own line. A length of -1 is
		 * used to encode a null array.
		 */
		...
		public boolean matches(URI uri, String requestMethod, Map<String, List<String>> requestHeaders) {
			return this.uri.equals(uri.toString()) && this.requestMethod.equals(requestMethod)
					&& new ResponseHeaders(uri, responseHeaders).varyMatches(varyHeaders.toMultimap(false), requestHeaders);
		}
	}
