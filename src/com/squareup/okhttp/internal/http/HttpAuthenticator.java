...
public final class HttpAuthenticator {
	 ...
	 * 
	 * @return true if credentials have been added to successorRequestHeaders and another request should be attempted.
	 */
	public static boolean processAuthHeader(int responseCode, RawHeaders responseHeaders, RawHeaders successorRequestHeaders, Proxy proxy, URL url)
			throws IOException {
	...
	/**
	 * Returns the authorization credentials that may satisfy the challenge. Returns null if a challenge header was not provided or if credentials
	 * were not available.
	 */
	private static String getCredentials(RawHeaders responseHeaders, String challengeHeader, Proxy proxy, URL url) throws IOException {
		...
		for (Challenge challenge : challenges) {
			...
			if (responseHeaders.getResponseCode() == HTTP_PROXY_AUTH) {
				...
				auth = Authenticator.requestPasswordAuthentication(proxyAddress.getHostName(), getConnectToInetAddress(proxy, url),
						proxyAddress.getPort(), url.getProtocol(), challenge.realm, challenge.scheme, url, Authenticator.RequestorType.PROXY);
			} else {
				auth = Authenticator.requestPasswordAuthentication(url.getHost(), getConnectToInetAddress(proxy, url), url.getPort(),
						url.getProtocol(), challenge.realm, challenge.scheme, url, Authenticator.RequestorType.SERVER);
			}
			if (auth == null) {
				continue;
			}
	...
	private static InetAddress getConnectToInetAddress(Proxy proxy, URL url) throws IOException {
		return (proxy != null && proxy.type() != Proxy.Type.DIRECT) ? ((InetSocketAddress) proxy.address()).getAddress() : InetAddress.getByName(url
				.getHost());
	}
