...
interface Transport {
	/**
	 * Returns an output stream where the request body can be written. The returned stream will of one of two types:
	 * <ul>
	 * <li><strong>Direct.</strong> Bytes are written to the socket and forgotten. This is most efficient, particularly for large request bodies. The
	 * returned stream may be buffered; the caller must call {@link #flushRequest} before reading the response.</li>
	 * <li><strong>Buffered.</strong> Bytes are written to an in memory buffer, and must be explicitly flushed with a call to
	 * {@link #writeRequestBody}. This allows HTTP authorization (401, 407) responses to be retransmitted transparently.</li>
	 * </ul>
	 */
	...
	boolean makeReusable(boolean streamReusable, OutputStream requestBodyOut, InputStream responseBodyIn);
}
