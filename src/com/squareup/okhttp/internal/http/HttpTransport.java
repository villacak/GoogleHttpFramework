...
public final class HttpTransport implements Transport {
	/**
	 * The maximum number of bytes to buffer when sending headers and a request body. When the headers and body can be sent in a single write, the
	 * request completes sooner. In one WiFi benchmark, using a large enough buffer sped up some uploads by half.
	 */
	private static final int MAX_REQUEST_BUFFER_LENGTH = 32768;

	/**
	 * The timeout to use while discarding a stream of input data. Since this is used for connection reuse, this timeout should be significantly less
	 * than the time it takes to establish a new connection.
	 */
	...
	/**
	 * This stream buffers the request headers and the request body when their combined size is less than MAX_REQUEST_BUFFER_LENGTH. By combining them
	 * we can save socket writes, which in turn saves a packet transmission. This is socketOut if the request size is large or unknown.
	 */
	 ...
	 * 
	 * <p>
	 * For streaming requests with a body, headers must be prepared <strong>before</strong> the output stream has been written to. Otherwise the body
	 * would need to be buffered!
	 * 
	 * <p>
	 * For non-streaming requests with a body, headers must be prepared <strong>after</strong> the output stream has been written to and closed. This
	 * ensures that the {@code Content-Length} header field receives the proper value.
	 */
	...
	/**
	 * Discards the response body so that the connection can be reused. This needs to be done judiciously, since it delays the current request in
	...
	/**
	 * An HTTP body with alternating chunk sizes and chunk bodies. Chunks are buffered until {@code maxChunkLength} bytes are ready, at which point
	 * the chunk is written and the buffer is cleared.
	 */
	private static final class ChunkedOutputStream extends AbstractHttpOutputStream {
		...
		/**
		 * Returns the amount of data that can be transmitted in a chunk whose total length (data+headers) is {@code dataPlusHeaderLength}. This is
		 * presumably useful to match sizes with wire-protocol packets.
		 */
