...
/**
 * This implementation uses HttpEngine to send requests and receive responses. This class may use multiple HttpEngines to follow redirects,
 * authentication retries, etc. to retrieve the final response body.
 * 
 * <h3>What does 'connected' mean?</h3> This class inherits a {@code connected} field from the superclass. That field is <strong>not</strong> used to
 * indicate not whether this URLConnection is currently connected. Instead, it indicates whether a connection has ever been attempted. Once a
 * connection has been attempted, certain properties (request header fields, request method, etc.) are immutable. Test the {@code connection} field on
 * this class for null/non-null to determine of an instance is currently connected to a server.
 */
public class HttpURLConnectionImpl extends HttpURLConnection {
	/**
	 * How many redirects should we follow? Chrome follows 21; Firefox, curl, and wget follow 20; Safari follows 16; and HTTP/1.0 recommends 5.
	 */
	...
	/**
	 * Returns the value of the field corresponding to the {@code fieldName}, or null if there is no such field. If the field has multiple values, the
	...
	/**
	 * Aggressively tries to get the final HTTP response, potentially making many HTTP requests in the process in order to cope with redirects and
	 * authentication.
	 */
	private HttpEngine getResponse() throws IOException {
		...
		while (true) {
			...
			if (responseCode == HTTP_MULT_CHOICE || responseCode == HTTP_MOVED_PERM || responseCode == HTTP_MOVED_TEMP
	...
	/**
	 * Sends a request and optionally reads a response. Returns true if the request was successfully executed, and false if the request can be
	...
	/**
	 * Returns the retry action to take for the current response headers. The headers, proxy and target URL or this connection may be adjusted to
	 * prepare for a follow up request.
	 */
	private Retry processResponseHeaders() throws IOException {
		Proxy selectedProxy = httpEngine.connection != null ? httpEngine.connection.getProxy() : requestedProxy;
		switch (getResponseCode()) {
			...
			case HTTP_UNAUTHORIZED:
				boolean credentialsFound = HttpAuthenticator.processAuthHeader(getResponseCode(), httpEngine.getResponseHeaders().getHeaders(),
						rawRequestHeaders, selectedProxy, url);
