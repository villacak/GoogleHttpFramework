...
/**
 * The HTTP status and unparsed header fields of a single HTTP message. Values are represented as uninterpreted strings; use {@link RequestHeaders}
 * and {@link ResponseHeaders} for interpreted headers. This class maintains the order of the header fields within the HTTP message.
 * 
 * <p>
 * This class tracks fields line-by-line. A field with multiple comma- separated values on the same line will be treated as a field with a single
 * value by this class. It is the caller's responsibility to detect and split on commas if their field permits multiple values. This simplifies use of
 * single-valued fields whose values routinely contain commas, such as cookies or dates.
 * 
 * <p>
...
public final class RawHeaders {
	...
	/**
	 * Returns a list of alternating names and values. Names are all lower case. No names are repeated. If any name has multiple values, they are
	...
	public List<String> toNameValueBlock() {
		...
		for (int i = 0; i < namesAndValues.size(); i += 2) {
			...
			if (name.equals("connection") || name.equals("host") || name.equals("keep-alive") || name.equals("proxy-connection")
