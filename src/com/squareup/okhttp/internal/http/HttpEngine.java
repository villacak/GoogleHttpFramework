 ...
 * <ol>
 * <li>It is created.
 * <li>The HTTP request message is sent with sendRequest(). Once the request is sent it is an error to modify the request headers. After sendRequest()
 * has been called the request body can be written to if it exists.
 * <li>The HTTP response message is read with readResponse(). After the response has been read the response headers and body can be read. All
 * responses have a response body input stream, though in some instances this stream is empty.
 * </ol>
 * 
 * <p>
 ...
 * 
 * <p>
 * This class may hold a socket connection that needs to be released or recycled. By default, this socket connection is held when the last byte of the
 * response is consumed. To release the connection when it is no longer required, use {@link #automaticallyReleaseConnectionToPool()}.
 */
public class HttpEngine {
	...
	/**
	 * True if the socket connection should be released to the connection pool when the response has been fully read.
	 */
	 ...
	 *            the client's supplied request headers. This class creates a private copy that it can mutate.
	 * @param connection
	 *            the connection used for an intermediate response immediately prior to this request/response pair, such as a same-host redirect. This
	 *            engine assumes ownership of the connection and must release it when it is unneeded.
	 */
	public HttpEngine(HttpURLConnectionImpl policy, String method, RawHeaders requestHeaders, Connection connection,
			RetryableOutputStream requestBodyOut) throws IOException {
		this.policy = policy;
		this.method = method;
	...
	/**
	 * Figures out what the response source will be, and opens a socket to that source if necessary. Prepares the request headers and gets ready to
	 * start writing the request body if it exists.
	 */
	...
	protected final void connect() throws IOException {
		...
		if (routeSelector == null) {
			...
			routeSelector = new RouteSelector(address, uri, policy.proxySelector, policy.connectionPool, Dns.DEFAULT);
		}
	...
	/**
	 * @param body
	 *            the response body, or null if it doesn't exist or isn't available.
	 */
	...
	/**
	 * Cause the socket connection to be released to the connection pool when it is no longer needed. If it is already unneeded, it will be pooled
	 * immediately. Otherwise the connection is held so that redirects can be handled by the same connection.
	 */
	...
	/**
	 * Releases this engine so that its resources may be either reused or closed. Also call {@link #automaticallyReleaseConnectionToPool} unless the
	 * connection will be used to follow a redirect.
	 */
	...
	public final boolean hasResponseBody() {
		...
		if ((responseCode < HTTP_CONTINUE || responseCode >= 200) && responseCode != HttpURLConnectionImpl.HTTP_NO_CONTENT
	...
	/**
	 * Returns the request status line, like "GET / HTTP/1.1". This is exposed to the application by {@link HttpURLConnectionImpl#getHeaderFields}, so
	...
	/**
	 * Returns the path to request, like the '/' in 'GET / HTTP/1.1'. Never empty, even if the request URL is. Includes the query component if it
	 * exists.
	 */
	...
	/**
	 * Returns true if the request line should contain the full URL with host and port (like "GET http://android.com/foo HTTP/1.1") or only the path
